...
import { accountSanctions, adminAuditLogs, adoptionCertifications, applicationEvents, applications, directAnimals, handoverReservations, members, moderationActions, notifications, posts, reports, returnRequests, sanctionAppeals, verificationRequests } from "../../../db/schema";
...
export async function GET() {
  ...
  const [applicationRows, registrationRows, verificationRows, reportRows, auditRows, certificationRows, appealRows] = await Promise.all([
    ...
    isAdmin?auth.db.select().from(adoptionCertifications).orderBy(desc(adoptionCertifications.createdAt)).limit(30):Promise.resolve([]),
    isAdmin?auth.db.select().from(sanctionAppeals).orderBy(desc(sanctionAppeals.createdAt)).limit(30):Promise.resolve([]),
  ]);
...
export async function POST(request: Request) {
  ...
  if(action==="adoption-certification-status"){if(auth.member.role!=="admin")return Response.json({error:"입양 인증 심사는 운영자만 할 수 있습니다."},{status:403});const status=clean(data.status,20)as"verified"|"rejected";if(!["verified","rejected"].includes(status))return Response.json({error:"상태를 확인해 주세요."},{status:400});const[row]=await auth.db.update(adoptionCertifications).set({status,reviewedBy:auth.user.userId}).where(eq(adoptionCertifications.id,id)).returning();if(!row)return Response.json({error:"인증 요청을 찾을 수 없습니다."},{status:404});await auth.db.insert(adminAuditLogs).values({actorId:auth.user.userId,action:`adoption-certification:${status}`,targetType:"adoption-certification",targetId:String(id),afterJson:JSON.stringify({status})});await auth.db.insert(notifications).values({memberId:row.memberId,type:"adoption_certification",title:"입양 인증 검토 결과",body:status==="verified"?"입양 인증이 완료되어 입양 일기를 작성할 수 있어요.":"입양 인증 자료를 다시 확인해 주세요.",href:"/adoption-verification"});return Response.json({row})}
  if(action==="appeal-status"){if(auth.member.role!=="admin")return Response.json({error:"이의제기 심사는 운영자만 할 수 있습니다."},{status:403});const status=clean(data.status,20)as"accepted"|"rejected",appeal=await auth.db.query.sanctionAppeals.findFirst({where:eq(sanctionAppeals.id,id)});if(!appeal||!["accepted","rejected"].includes(status))return Response.json({error:"이의제기와 상태를 확인해 주세요."},{status:400});const[row]=await auth.db.update(sanctionAppeals).set({status,reviewedBy:auth.user.userId}).where(eq(sanctionAppeals.id,id)).returning();await auth.db.update(accountSanctions).set({status:status==="accepted"?"lifted":"confirmed"}).where(eq(accountSanctions.id,appeal.sanctionId));if(status==="accepted")await auth.db.update(members).set({sanctioned:false}).where(eq(members.id,appeal.memberId));await auth.db.insert(adminAuditLogs).values({actorId:auth.user.userId,action:`appeal:${status}`,targetType:"sanction-appeal",targetId:String(id),afterJson:JSON.stringify({status})});return Response.json({row})}
