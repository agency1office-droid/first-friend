...
test("uses a contextual animal detail topbar", async () => {
  ...
  assert.match(readinessQuiz, /new URL\(document\.referrer\)\.origin === window\.location\.origin/);
  ...
  assert.match(readinessQuiz, /반려동물 친구를 만나러 가볼까요/);
  assert.match(readinessQuiz, /if \(phase === "result"\)/);
  assert.match(readinessQuiz, /수료증 받기/);
  ...
  assert.match(readinessQuiz, /if \(phase === "intro"\) \{ setSpecies\(null\); setAnswers\(\{\}\); setFirstAnswers\(\{\}\); setFeedbackAnswer\(null\); setRetryCorrectAnswerVisible\(false\); setStep\(0\); setPhase\("species"\); return; \}/);
  assert.match(readinessQuiz, /if \(phase === "questions" && feedbackAnswer !== null\) \{ setFeedbackAnswer\(null\); setPendingAnswer\(null\); setRetryCorrectAnswerVisible\(true\); setRevealedAnswer\(null\); return; \}/);
  assert.match(readinessQuiz, /const \[retryCorrectAnswerVisible, setRetryCorrectAnswerVisible\] = useState\(false\);/);
  assert.doesNotMatch(readinessQuiz, /힌트|questionHints|ff-readiness-question-hint/);
  assert.match(readinessQuiz, /revealedAnswer/);
  assert.match(readinessQuiz, /data-correct=\{correctVisible \|\| undefined\}/);
  ...
  assert.match(readinessQuiz, /function skipFeedback\(\)/);
  ...
  assert.match(readinessQuiz, /BottomSheetContent/);
  assert.match(readinessQuiz, /수료증 받기/);
  assert.match(readinessQuiz, /닫기/);
  assert.match(readinessQuiz, /공유하기/);
  assert.match(readinessQuiz, /shareCertificate/);
  assert.match(readinessQuiz, /authState === "authenticated" \? "공유하기" : authState === "checking" \? "확인 중" : "로그인"/);
  assert.match(readinessQuiz, /disabled=\{authState === "checking"\}/);
  assert.match(readinessQuiz, /준비를 완벽하게 마쳤어요/);
  assert.match(readinessQuiz, /questions\.length}문제 중 \{correctCount\}문제를 맞혔어요/);
  ...
  assert.match(readinessQuiz, /로그인하면 수료 기록과 결과를 저장할 수 있어요/);
  assert.match(readinessQuiz, /BottomSheetContent layerIndex=\{100\}/);
  assert.match(readinessQuiz, /aria-label="수료증 내용"/);
  assert.match(readinessQuiz, /showCloseButton=\{false\}/);
  ...
  assert.match(readinessQuiz, /passed \? <BottomSheetTrigger asChild><ActionButton size="large" variant="brandSolid" className="ff-grow">수료증 받기<\/ActionButton><\/BottomSheetTrigger> : <ActionButton size="large" variant="brandSolid" className="ff-grow" onClick=\{restartQuiz\}>다시 풀기<\/ActionButton>/);
  assert.match(readinessQuiz, /넘어가기/);
  assert.match(readinessQuiz, /variant="neutralWeak" className="ff-grow" onClick=\{skipFeedback\}>넘어가기<\/ActionButton><ActionButton size="large" variant="brandSolid" className="ff-grow" onClick=\{previous\}>다시 풀기/);
