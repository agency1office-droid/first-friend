...
import { IconChevronLeftLine, IconChevronDownLine, IconPawprintLine, IconSlider2HorizontalLine, IconXmarkLine } from "@karrotmarket/react-monochrome-icon";
    ...
    {open && <div className="ff-all-filter-overlay" role="dialog" aria-modal="true" aria-label="전체 필터" onWheelCapture={event => event.stopPropagation()} onTouchMoveCapture={event => event.stopPropagation()}>
      ...
      <div className="ff-all-filter-content ff-filter-reference-content">
        {loading && <div className="ff-all-filter-loading"><LoadingIndicator label="필터를 불러오는 중" /></div>}{error && <p className="ff-all-filter-error">{error}</p>}{options && <>
          <section className="ff-filter-reference-section"><SectionHeading title="동물 종류 · 품종" icon={<IconPawprintLine />} /><div className="ff-filter-species-grid">{(["dog", "cat"] as const).map(value => <button type="button" key={value} className="ff-filter-species-card" data-selected={species === value || undefined} onClick={() => { if (species === value) { setSpecies("all"); setBreeds([]); setColor("all"); setSizeGroup("all"); setShowBreeds(false); return; } setSpecies(value); setColor(current => colorsForSpecies(value).includes(current as never) ? current : "all"); setShowBreeds(true); setBreeds(current => current.filter(key => { const breed = options.breeds.find(item => item.key === key); return !breed || normalizedSpecies(breed.species) === value; })); }}><span className="ff-filter-species-illustration" aria-hidden>{value === "dog" ? "🐶" : "🐱"}</span><strong>{value === "dog" ? "강아지" : "고양이"}</strong><small>{options.breeds.filter(item => normalizedSpecies(item.species) === value).reduce((sum, item) => sum + item.count, 0).toLocaleString("ko-KR")}마리</small></button>)}</div>{species !== "all" && <div className="ff-filter-breed-toolbar"><strong>품종</strong><button type="button" data-expanded={showBreeds || undefined} aria-expanded={showBreeds} onClick={() => setShowBreeds(current => !current)} aria-label={showBreeds ? "품종 접기" : "품종 펼치기"}><Icon svg={<IconChevronDownLine />} /></button></div>}{showBreeds && <><TextField className="ff-filter-search"><TextFieldInput value={breedQuery} onChange={event => setBreedQuery(event.target.value)} placeholder="품종을 검색해보세요" /></TextField><div className="ff-filter-breed-list" role="list" aria-label="품종 목록"><button type="button" className="ff-filter-breed-any" data-selected={!breeds.length || undefined} onClick={() => setBreeds([])}><strong>모든 품종</strong><small>상관없음</small></button>{visibleBreeds.slice(0, 100).map(item => <Checkbox key={item.key} checked={breeds.includes(item.key)} onCheckedChange={() => toggle(setBreeds)(item.key)} label={<span className="ff-breed-filter-label"><strong>{humanize(item.label)}</strong><small>{item.count.toLocaleString("ko-KR")}마리</small></span>} />)}</div></>}</section>
          ...
          <section className="ff-filter-reference-section"><SectionHeading title="나이" /><div className="ff-filter-choice-grid"><button type="button" className="ff-filter-choice ff-filter-age-choice" data-selected={ageGroup === "all" || undefined} onClick={() => setAgeGroup("all")}><strong>상관없음</strong><small>모든 나이</small></button>{ageOptions.map(([value, title, range]) => <button type="button" key={value} className="ff-filter-choice ff-filter-age-choice" data-selected={ageGroup.split(",").includes(value) || undefined} onClick={() => setAgeGroup(current => { const values = current === "all" ? [] : current.split(","); const next = values.includes(value) ? values.filter(item => item !== value) : [...values, value]; return next.join(",") || "all"; })}><strong>{title}</strong><small>{range}</small></button>)}</div></section>
          {species !== "all" && <section className="ff-filter-reference-section"><SectionHeading title="크기" /><div className="ff-filter-choice-grid"><button type="button" className="ff-filter-choice ff-filter-size-choice" data-selected={sizeGroup === "all" || undefined} onClick={() => setSizeGroup("all")}><strong>상관없음</strong></button>{sizeOptions().map(([value, label]) => <button type="button" key={value} className={`ff-filter-choice ff-filter-size-choice is-${value}`} data-selected={sizeGroup.split(",").includes(value) || undefined} onClick={() => setSizeGroup(current => { const values = current === "all" ? [] : current.split(","); const next = values.includes(value) ? values.filter(item => item !== value) : [...values, value]; return next.join(",") || "all"; })}><strong>{label}</strong></button>)}</div></section>}
          <section className="ff-filter-reference-section"><SectionHeading title="성별" /><div className="ff-filter-choice-grid"><button type="button" className="ff-filter-choice ff-filter-sex-choice" data-selected={!sex.length || undefined} onClick={() => setSex([])}><strong>상관없음</strong></button>{orderedSex(options.sex).map(value => <button type="button" key={value} className="ff-filter-choice ff-filter-sex-choice" data-selected={sex.includes(value) || undefined} onClick={() => setSex(current => current.includes(value) ? current.filter(item => item !== value) : [...current, value])}><strong>{humanize(value)}</strong></button>)}</div></section>
          <section className="ff-filter-reference-section"><SectionHeading title="중성화 여부" /><div className="ff-filter-choice-grid"><button type="button" className="ff-filter-choice ff-filter-neuter-choice" data-selected={!neutered.length || undefined} onClick={() => setNeutered([])}><strong>상관없음</strong></button>{[["yes", "중성화 완료"], ["no", "중성화 안 됨"]].map(([value, label]) => <button type="button" key={value} className="ff-filter-choice ff-filter-neuter-choice" data-selected={neutered.includes(label) || undefined} onClick={() => setNeutered(current => current.includes(label) ? current.filter(item => item !== label) : [...current, label])}><strong>{label}</strong></button>)}</div></section>
        </>}
