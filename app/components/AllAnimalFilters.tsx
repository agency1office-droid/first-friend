    ...
    {open && <div className="ff-all-filter-overlay" role="dialog" aria-modal="true" aria-label="전체 필터">
      ...
      <div className="ff-all-filter-content ff-filter-reference-content">
        {loading && <div className="ff-all-filter-loading"><LoadingIndicator label="필터를 불러오는 중" /></div>}{error && <p className="ff-all-filter-error">{error}</p>}{options && <>
          ...
          <section className="ff-filter-reference-section"><SectionHeading title="나이" icon={<IconCalendarLine />} /><div className="ff-filter-choice-grid">{ageOptions.map(([value, title, range]) => <button type="button" key={value} className="ff-filter-choice ff-filter-age-choice" data-selected={ageGroup === value || undefined} onClick={() => setAgeGroup(ageGroup === value ? "all" : value)}><span className="ff-filter-choice-icon ff-filter-age-icon" aria-hidden>{ageIcon(value)}</span><strong>{title}</strong><small>{range}</small></button>)}</div></section>
          <section className="ff-filter-reference-section"><SectionHeading title={species === "dog" ? "강아지 크기" : species === "cat" ? "고양이 크기" : "크기"} icon={<IconNeedleScaleLine />} /><div className="ff-filter-choice-grid">{sizeOptions(species).map(([value, label]) => <button type="button" key={value} className={`ff-filter-choice ff-filter-size-choice is-${value}`} data-selected={sizeGroup === value || undefined} onClick={() => setSizeGroup(sizeGroup === value ? "all" : value)}><span className="ff-filter-choice-icon ff-filter-size-icon" aria-hidden><IconPawprintLine /></span><strong>{label}</strong></button>)}</div></section>
          <section className="ff-filter-reference-section"><SectionHeading title="성별" icon={<IconMalesymbolFemalesymbolLine />} /><div className="ff-filter-choice-grid">{orderedSex(options.sex).map(value => <button type="button" key={value} className="ff-filter-choice ff-filter-sex-choice" data-selected={sex.includes(value) || undefined} onClick={() => { if (value === "미상" || sex.includes(value)) setSex([]); else setSex([value]); }}><span className={`ff-filter-choice-icon ff-filter-sex-icon is-${value === "수컷" ? "male" : value === "암컷" ? "female" : "unknown"}`} aria-hidden>{sexIcon(value)}</span><strong>{humanize(value)}</strong></button>)}</div></section>
