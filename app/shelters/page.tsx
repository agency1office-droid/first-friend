...
export default async function SheltersPage() {
  ...
  return <div className="ff-page">
    ...
    <section className="ff-section">
      {shelters.length ? <List>{shelters.map((shelter, index) => <div key={shelter.id}><ListItem prefix={<IconHospitalcrossBuildingLine/>} title={shelter.name} detail={`${shelter.organization} · ${shelter.animals}\n${shelter.address}\n평일 ${shelter.hours} · ${shelter.closed}`} suffix={<a className="ff-shelter-phone" href={`tel:${shelter.phone.replace(/[^\d+]/g, "")}`} aria-label={`${shelter.name} 전화하기`}><IconPhoneLine/></a>}/>{index < shelters.length - 1 && <ListDivider/>}</div>)}</List> : <div className="ff-empty">보호센터 정보를 불러오지 못했어요. 잠시 후 다시 확인해 주세요.</div>}
    </section>
  </div>;
}
