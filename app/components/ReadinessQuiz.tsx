...
export function ReadinessQuiz({ onClose = () => {} }: { onClose?: () => void }) {
  ...
  const isFeedbackOpen = hasAnswered && !isAnswerCorrect;
  ...
  return <div className={`ff-readiness ff-readiness-${phase}`}>
    ...
    {!isFeedbackOpen && <footer className={`ff-readiness-actions${phase === "intro" || phase === "species" || phase === "questions" || phase === "result" ? " is-single" : ""}`}>{phase === "result" ? <ActionButton size="large" className="ff-grow" asChild><a href="/find">입양할 친구 찾기</a></ActionButton> : phase === "intro" ? <ActionButton size="large" className="ff-grow" onClick={next}>시작하기</ActionButton> : phase === "species" ? <ActionButton size="large" variant="brandSolid" className="ff-grow" disabled={species === null} onClick={next}>다음</ActionButton> : <ActionButton size="large" variant="brandSolid" className="ff-grow" disabled={pendingAnswer === null} onClick={next}>{step === questions.length - 1 ? "결과 보기" : "다음"}</ActionButton>}</footer>}
  </div>;
}
