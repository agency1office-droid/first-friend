...
export default async function StoriesPage({searchParams}:{searchParams:Promise<Record<string,string|string[]|undefined>>}){
 ...
 return <div className="ff-page ff-board-page">
  ...
  {error?<Callout tone="critical" description={error}/>:<><div className="ff-board-total">전체 {result!.total}개</div><StoryFeed key={params.toString()} initial={result!} query={params.toString()}/></>}
 </div>;
}
