...
import { IconArrowCounterclockwiseCircularLine, IconChevronDownLine } from "@karrotmarket/react-monochrome-icon";
...
export function AnimalFilterBar({ filters, activeCount, setFilter, resetFilters }: { filters: AnimalFeedFilters; location: { lat: number; lng: number } | null; hasLocation: boolean; activeCount: number; setFilter: <K extends keyof AnimalFeedFilters>(key: K, value: AnimalFeedFilters[K]) => void; resetFilters: () => void }) {
  ...
  return <div className="ff-animal-filter-wrap" aria-label="보호동물 목록 필터"><div className="ff-animal-filter-scroll" ref={scrollRef} onPointerDown={startDrag} onClickCapture={swallowClickAfterDrag}><AllAnimalFilters activeCount={activeCount} filters={filters} setFilter={setFilter} resetFilters={resetFilters}/>{/* 초기화는 줄 끝에 두면 잘려서 있는지 알 수 없으므로, 필터가 켜진 동안 필터 아이콘 바로 옆에 둡니다. */}{activeCount > 0 && <Chip.Button className="ff-animal-filter-reset" variant="outlineWeak" size="medium" onClick={resetFilters} aria-label="필터 전체 초기화"><Chip.PrefixIcon><Icon svg={<IconArrowCounterclockwiseCircularLine />} /></Chip.PrefixIcon><Chip.Label>초기화</Chip.Label></Chip.Button>}<SimpleOptionSheet title="정렬 기준" description="가까운 보호소 또는 최근 등록된 순서로 볼 수 있어요." value={sortValue} options={sortOptions} active={sortValue === "distance"} onChange={value => setFilter("sort", value as AnimalFeedFilters["sort"])} onReset={() => setFilter("sort", "distance")}/><SimpleOptionSheet title="보호 단계" description="현재 보호 절차에 따라 친구를 골라볼 수 있어요." value={filters.publicStatus} options={statusOptions} active={filters.publicStatus !== "all"} onChange={value => setFilter("publicStatus", value as AnimalFeedFilters["publicStatus"])} onReset={() => setFilter("publicStatus", "all")}/><SimpleOptionSheet title="건강" description="보호소 특징 메모를 바탕으로 건강 상태에 따라 친구를 골라볼 수 있어요." value={filters.health} options={healthOptions} active={filters.health !== "all"} onChange={value => setFilter("health", value as AnimalFeedFilters["health"])} onReset={() => setFilter("health", "all")}/><SpeciesFilterSheet value={filters.species} onChange={value => { setFilter("species", value); setFilter("breedKeys", []); }} onReset={() => { setFilter("species", "all"); setFilter("breedKeys", []); }}/></div></div>;
}
