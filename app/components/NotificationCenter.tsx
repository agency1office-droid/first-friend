...
export function NotificationCenter() {
  ...
  useEffect(load, []);
  const readAll = async () => { const response = await fetch("/api/notifications", { method:"POST" }); if (response.ok) { setItems(current => current.map(item => ({...item, read:true}))); window.dispatchEvent(new Event("ff-notifications-change")); } };
