...
export default async function MyPage() {
  ...
  if (user) {
    try {
      ...
      const [readiness, applicationRows, favoriteCount, postCount, reportCount, registrationCount, searchCount, unreadCount] = await Promise.all([
        ...
        client.from("posts").select("id", { count: "exact", head: true }).eq("member_id", user.userId).neq("status", "deleted"),
  ...
  return <div className="ff-page">
    ...
    {user&&<ActionButton asChild variant="neutralWeak"><Link prefetch={false} href="/mypage/contact">고객 문의·마케팅 수신 설정</Link></ActionButton>}
    <section className="ff-section"><h2 className="ff-section-title" style={{ marginBottom: 10 }}>나의 활동</h2><List><ListLinkItem href="/mypage/favorites" prefix={<IconHeartLine/>} title="관심 친구와 저장 검색" detail={dashboard ? `${dashboard.favorites}마리 · 알림 조건 ${dashboard.searches}개` : undefined} suffix={<IconChevronRightLine/>}/><ListDivider/><ListLinkItem href="/notifications" prefix={<IconArticleLine/>} title="알림함" detail={dashboard ? `읽지 않은 알림 ${dashboard.unread}개` : undefined} suffix={<IconChevronRightLine/>}/><ListDivider/><ListLinkItem href="/readiness" prefix={<IconCheckmarkShieldFill/>} title="입양 준비 시험과 교육" suffix={<IconChevronRightLine/>}/><ListDivider/><ListLinkItem href="/stories/manage" prefix={<IconArticleLine/>} title="나의 공개 이야기" suffix={<IconChevronRightLine/>}/><ListDivider/><ListLinkItem href={dashboard?.reports ? "/lost-found?mine=1" : "/lost-found"} prefix={<IconMagnifyingglassLine/>} title="실종·발견 신고와 알림" detail={dashboard ? `${dashboard.reports}건 등록 · 접수 뒤 연결 페이지에서 관리` : undefined} suffix={<IconChevronRightLine/>}/><ListDivider/><ListLinkItem href="/foster" prefix={<IconHousePlusLine/>} title="임시보호 동물 직접 등록" detail={dashboard ? `${dashboard.registrations}건 등록` : undefined} suffix={<IconChevronRightLine/>}/><ListDivider/><ListLinkItem href="/verification" prefix={<IconPersonShieldLine/>} title="임시보호자·보호단체 역할 인증" suffix={<IconChevronRightLine/>}/><ListDivider/><ListLinkItem href="/operations" prefix={<IconGearLine/>} title="보호처 운영 콘솔" detail="인증 보호처와 운영자만 이용" suffix={<IconChevronRightLine/>}/></List></section>
    ...
    <section className="ff-section"><h2 className="ff-section-title" style={{marginBottom:10}}>가족·상담·도움 관리</h2><div className="ff-service-grid"><Link prefetch={false} href="/mypage/favorites"><IconHeartLine/><strong>관심 친구</strong><span>찜한 친구 다시 보기</span></Link><Link prefetch={false} href="/mypage/searches"><IconMagnifyingglassLine/><strong>검색 알림</strong><span>조건 켜기·끄기·삭제</span></Link><Link prefetch={false} href="/mypage/family"><IconHeartLine/><strong>가족 상의</strong><span>함께 본 친구와 의견</span></Link><Link prefetch={false} href="/mypage/messages"><IconArticleLine/><strong>메시지함</strong><span>입양 상담과 인계 기록</span></Link><Link prefetch={false} href="/adoption-verification"><IconCheckmarkShieldFill/><strong>외부 입양 인증</strong><span>보호소 코드·증빙 검토</span></Link><Link prefetch={false} href="/appeal"><IconPersonShieldLine/><strong>제재 이의제기</strong><span>운영자 재검토 요청</span></Link><Link prefetch={false} href="/stories/manage"><IconArticleLine/><strong>글 관리</strong><span>수정·삭제·공유</span></Link><Link prefetch={false} href="/mypage/help"><IconHousePlusLine/><strong>도움 기록</strong><span>후원·굿즈·물품 상태</span></Link><Link prefetch={false} href="/shelters/manage"><IconGearLine/><strong>보호소 운영</strong><span>소식·봉사·필요 물품</span></Link><Link prefetch={false} href="/mypage/reputation"><IconCheckmarkShieldFill/><strong>포인트·봉사 배지</strong><span>매칭 도움과 완료 봉사 기록</span></Link><Link prefetch={false} href="/volunteer"><IconHousePlusLine/><strong>유형별 봉사 찾기</strong><span>청소·사진·이동·의료·돌봄</span></Link></div></section>
  </div>;
}
