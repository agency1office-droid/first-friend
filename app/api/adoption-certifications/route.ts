...
export async function POST(request: Request) {
  ...
  const shelterName = clean(data.shelterName, 120), animalName = clean(data.animalName, 80), verificationCode = clean(data.verificationCode, 80), evidenceKey = clean(data.evidenceKey, 240);
  if (!shelterName || !animalName || verificationCode.length < 6 || !ownedUploadKey(evidenceKey, user.userId, ["private-evidence"])) return Response.json({ error: "보호소·동물·인증코드·본인 입양 증빙을 모두 확인해 주세요." }, { status: 400 });
