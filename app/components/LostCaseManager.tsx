...
export function LostCaseManager({ id }:{id:number}) {
  ...
  async function load(){const response=await fetch(`/api/lost-found/${id}`);if(response.status===401){location.href=`/signin-with-chatgpt?return_to=%2Flost-found%2F${id}`;return}const body=await response.json();if(response.ok)setData(body);else setError(body.error||"불러오지 못했어요.")}
  useEffect(()=>{let active=true;void fetch(`/api/lost-found/${id}`).then(async response=>({response,body:await response.json()})).then(({response,body})=>{if(!active)return;if(response.status===401){location.href=`/signin-with-chatgpt?return_to=%2Flost-found%2F${id}`;return}if(response.ok)setData(body);else setError(body.error||"불러오지 못했어요.")});return()=>{active=false}},[id]);
