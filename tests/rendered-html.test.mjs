...
test("uses a contextual animal detail topbar", async () => {
  ...
  assert.match(calculator, /href=\{`\/pet-cost-calculator\?species=\$\{initialSpecies\}`\}/);
  ...
  assert.match(legacyCostPage, /redirect\(`\/pet-cost-calculator\$\{species\}`\)/);
  ...
  assert.match(readinessStyles, /\.ff-readiness-chapter \{[^}]*gap: 24px;[^}]*margin-top: 32px;[^}]*padding: 0 16px;/);
  ...
  assert.match(readinessQuiz, /if \(phase === "intro"\) \{ setSpecies\(null\); setAnswers\(\{\}\); setFirstAnswers\(\{\}\); setFeedbackAnswer\(null\); setRetryAnswerHintsVisible\(false\); setStep\(0\); setPhase\("species"\); return; \}/);
