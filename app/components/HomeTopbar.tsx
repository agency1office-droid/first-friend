...
import { IconBellLine,IconChevronDownLine,IconChevronRightLine,IconHorizline3VerticalLine } from "@karrotmarket/react-monochrome-icon";
...
export function HomeTopbar(){
  ...
  return <header className="ff-topbar ff-home-topbar"><BottomSheetRoot open={regionOpen} onOpenChange={setRegionOpen}><BottomSheetTrigger asChild><button className="ff-home-region" type="button" aria-label={`현재 지역 ${region}, 지역 변경`}><span>{region}</span><IconChevronDownLine aria-hidden/></button></BottomSheetTrigger><BottomSheetContent title="내 지역 설정" description="정확한 주소 대신 시·군·구 또는 동네까지만 입력해요."><BottomSheetBody><div className="ff-home-region-form"><TextField label="활동 지역"><TextFieldInput value={draft} onChange={event=>setDraft(event.target.value)} placeholder="예: 서울 마포구"/></TextField><ActionButton onClick={save}>이 지역에서 찾기</ActionButton></div></BottomSheetBody></BottomSheetContent></BottomSheetRoot><div className="ff-top-actions"><BottomSheetRoot><BottomSheetTrigger asChild><button className="ff-icon-link" type="button" aria-label="전체 메뉴"><IconHorizline3VerticalLine aria-hidden/></button></BottomSheetTrigger><BottomSheetContent title="퍼스트 프렌드 메뉴" description="입양과 지역 동물 도움 기능을 찾아보세요."><BottomSheetBody><nav className="ff-home-menu" aria-label="전체 메뉴">{menu.map(([label,href])=><a href={href} key={href}><span>{label}</span><IconChevronRightLine aria-hidden/></a>)}</nav></BottomSheetBody></BottomSheetContent></BottomSheetRoot><a className="ff-icon-link" href="/notifications" aria-label="알림 확인"><IconBellLine aria-hidden/></a></div></header>
}
