...
async function performAction(request: Request) {
  ...
  if (action === "post-delete") { if (!admin) return Response.json({ error: "게시물 삭제는 운영자만 할 수 있습니다." }, { status: 403 }); const { data: before } = await c.from("posts").select("*").eq("id", id).maybeSingle().throwOnError(); if (!before) return Response.json({ error: "게시물을 찾지 못했어요." }, { status: 404 }); if (before.status === "deleted") return Response.json({ error: "이미 삭제된 게시물이에요." }, { status: 409 }); const { data: row } = await c.from("posts").update({ status: "deleted", updated_at: new Date().toISOString() }).eq("id", id).eq("status", before.status).select("*").single().throwOnError(); await audit(c, auth.user.userId, "post:deleted", "post", String(id), before, row); return Response.json({ row: row ? map(row) : null }); }
