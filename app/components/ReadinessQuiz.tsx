...
export function ReadinessQuiz({ onClose = () => {} }: { onClose?: () => void }) {
  ...
  return <div className={`ff-readiness ff-readiness-${phase}`}>
    ...
    <footer className={`ff-readiness-actions${phase === "intro" || phase === "species" ? " is-single" : ""}`}>{phase === "result" ? <><ActionButton size="large" variant="neutralWeak" onClick={() => { setPhase("intro"); setShowResult(false); setStep(0); setSpecies(null); }}>다시 확인하기</ActionButton><ActionButton size="large" className="ff-grow" asChild><a href="/find">입양할 친구 찾기</a></ActionButton></> : phase === "intro" ? <ActionButton size="large" className="ff-grow" onClick={next}>시작하기</ActionButton> : phase === "species" ? <ActionButton size="large" variant="neutralWeak" className="ff-grow" disabled={species === null} onClick={next}>다음</ActionButton> : <><ActionButton size="large" variant="neutralWeak" onClick={previous}>이전</ActionButton><ActionButton size="large" variant="neutralWeak" className="ff-grow" disabled={answers[questionIndex] === undefined} onClick={next}>{step === questions.length - 1 ? "결과 보기" : "다음"}</ActionButton></>}</footer>
  </div>;
}
