...
- 한 실행은 최대 240초 동안 새 작업을 가져오며 동시에 최대 12장, 최대 5,000건을 처리한다(함수 한도 300초에서 다운로드 타임아웃 15초와 변환·업로드 여유를 남긴 값). 처리 중인 배치는 마무리하므로 실제 응답은 더 늦을 수 있다. 남은 작업은 다음 실행에서 이어간다. 하루 유입(동물당 사진 2장)이 이 처리량을 넘어 대기 건수가 쌓이면 아래 백필을 실행한다.
