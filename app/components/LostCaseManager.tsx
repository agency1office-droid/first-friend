...
export function LostCaseManager({ id }:{id:number}) {
  ...
  async function submit(event:React.FormEvent<HTMLFormElement>,action:string){event.preventDefault();const formElement=event.currentTarget, form=new FormData(formElement);const payload=Object.fromEntries(form.entries());const response=await fetch(`/api/lost-found/${id}`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({...payload,action})});const body=await response.json();if(!response.ok)setError(body.error||"저장하지 못했어요.");else{formElement.reset();await load()}}
