import { directAnimals } from "../../../db/schema";
...
export async function POST(request: Request) {
  ...
  const name = clean(data.name, 60), species = clean(data.species, 20), region = clean(data.region, 80), rescueStory = clean(data.rescueStory), adoptionTerms = clean(data.adoptionTerms), imageKey = clean(data.imageKey, 240);
