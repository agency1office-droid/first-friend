...
export async function POST(request: Request) {
  ...
  if (!["public-media", "drawing-board", "role-verification", "adoption-verification"].includes(purpose)) return Response.json({ error: "파일 용도를 확인해 주세요." }, { status: 400 });
  ...
  const contentHash = Array.from(new Uint8Array(await crypto.subtle.digest("SHA-256", content)), byte => byte.toString(16).padStart(2, "0")).join("");
  const guard = await beginIdempotentRequest("uploads", requestSubject(request, user.userId), request, `${purpose}:${file.type}:${contentHash}`);
