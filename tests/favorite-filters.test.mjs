...
test('favorite filters intersect, reset and preserve removal behavior', async () => {
  ...
  const click=(group,label)=>{const sheet=nodes(render()).find(n=>n.type==='sheet'&&n.props.title===group);sheet.props.onChange(sheet.props.options.find(o=>o[1]===label)[0]);};
  assert.equal(cards().length,3);
  click('동물 종류','고양이');assert.equal(cards()[0].props.animal.id,'cat');
  click('보호 단계','보호자 확인 공고 중');assert.equal(cards().length,0);
  nodes(render()).find(n=>n.type==='button'&&n.props.children.props.children==='전체 초기화').props.onClick();
  assert.equal(cards().length,3);
