...
export function ReadinessQuiz({ onClose = () => {} }: { onClose?: () => void }) {
  ...
  return <div className={`ff-readiness ff-readiness-${phase}`}>
    ...
    {phase === "questions" && <section className="ff-readiness-chapter" aria-labelledby="readiness-question-title"><h2 id="readiness-question-title">{question.question}</h2>{hasAnswered && <p className="ff-readiness-selected-answer">내가 고른 답변 · {question.options[selectedAnswer]}</p>}{hasAnswered ? <div className={`ff-readiness-feedback${isAnswerCorrect ? " is-correct" : " is-incorrect"}`} role="status" aria-live="polite"><strong><span className="ff-readiness-feedback-mark" aria-hidden>{isAnswerCorrect ? <IconCheckmarkCircleFill /> : <IconXmarkCircleFill />}</span>{isAnswerCorrect ? "정답이에요" : "아직 헷갈릴 수 있어요"}</strong><p>{isAnswerCorrect ? `잘 알고 있어요. ${question.explanation}` : `정답은 “${question.options[question.answer]}”예요. ${question.explanation}`}</p></div> : <fieldset className="ff-quiz-question ff-quiz-question-single"><legend className="ff-visually-hidden">{question.question}</legend>{question.options.map((option, optionIndex) => <label key={option}><input type="radio" name={`readiness-chapter-${questionIndex}`} checked={answers[questionIndex] === optionIndex} onChange={() => setAnswers((current) => ({ ...current, [questionIndex]: optionIndex }))} /><span className="ff-quiz-option-label">{option}</span></label>)}</fieldset>}</section>}
    ...
    <footer className={`ff-readiness-actions${phase === "intro" || phase === "species" || phase === "questions" || phase === "result" ? " is-single" : ""}`}>{phase === "result" ? <ActionButton size="large" className="ff-grow" asChild><a href="/find">입양할 친구 찾기</a></ActionButton> : phase === "intro" ? <ActionButton size="large" className="ff-grow" onClick={next}>시작하기</ActionButton> : phase === "species" ? <ActionButton size="large" variant="brandSolid" className="ff-grow" disabled={species === null} onClick={next}>다음</ActionButton> : <ActionButton size="large" variant="brandSolid" className="ff-grow" disabled={answers[questionIndex] === undefined} onClick={next}>{step === questions.length - 1 ? "결과 보기" : "다음 문제"}</ActionButton>}</footer>
  </div>;
}
