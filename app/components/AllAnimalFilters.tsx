...
function SnapRange({ label, value, max, unit, onChange }: { label: string; value: [number, number]; max: number; unit: string; onChange: (value: [number, number]) => void }) {
  const maxValue = value[1], point = maxValue === max ? 0 : maxValue + 1;
  const ticks = unit === "살" ? [0, 5, 10, 15, 20] : [0, 10, 20, 30, 40, 50];
...
export function AllAnimalFilters({ activeCount, filters, setFilter, resetFilters }: Props) {
  ...
  const openPanel = () => {
    ...
    setAgeRange(nextAge === 0 ? [0, 20] : [0, nextAge - 1]); setWeightRange(nextWeight === 0 ? [0, 50] : [0, nextWeight - 1]); setBreedQuery(""); setShowBreeds(Boolean(filters.species !== "all")); setError(""); setOpen(true);
  };
  useEffect(() => {
    ...
    {open && <div className="ff-all-filter-overlay" role="dialog" aria-modal="true" aria-label="전체 필터">
      ...
      <div className="ff-all-filter-content ff-filter-reference-content">
        {loading && <div className="ff-all-filter-loading"><LoadingIndicator label="필터를 불러오는 중" /></div>}{error && <p className="ff-all-filter-error">{error}</p>}{options && <>
          ...
          <section className="ff-filter-reference-section"><SectionHeading title="나이" icon={<IconCalendarLine />} /><SnapRange label="나이" value={ageRange} max={20} unit="살" onChange={setAgeRange} /></section>
          <section className="ff-filter-reference-section"><SectionHeading title="체중" icon={<IconCheckmarkScaleLine />} /><SnapRange label="체중" value={weightRange} max={50} unit="kg" onChange={setWeightRange} /></section>
