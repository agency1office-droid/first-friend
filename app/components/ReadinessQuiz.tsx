...
export function ReadinessQuiz({ onClose = () => {} }: { onClose?: () => void }) {
  ...
  return <div className={`ff-readiness ff-readiness-${phase}`}>
    ...
    {phase === "questions" && <section className="ff-readiness-chapter" aria-labelledby="readiness-question-title"><h2 id="readiness-question-title">{question.question}</h2>{hasAnswered ? <div className={`ff-readiness-feedback${isAnswerCorrect ? " is-correct" : " is-incorrect"}`} role="status" aria-live="polite"><strong>{isAnswerCorrect ? "맞아요" : "다시 확인해 볼게요"}</strong><p>{isAnswerCorrect ? `잘 알고 있어요. ${question.explanation}` : `이 내용은 다시 확인해 볼게요. ${question.explanation} 정답은 “${question.options[question.answer]}”예요.`}</p></div> : <fieldset className="ff-quiz-question ff-quiz-question-single"><legend className="ff-visually-hidden">{question.question}</legend>{question.options.map((option, optionIndex) => <label key={option}><input type="radio" name={`readiness-chapter-${questionIndex}`} checked={answers[questionIndex] === optionIndex} onChange={() => setAnswers((current) => ({ ...current, [questionIndex]: optionIndex }))} /><span className="ff-quiz-option-label">{option}</span></label>)}</fieldset>}</section>}
