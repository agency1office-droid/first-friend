...
export async function POST(request: Request) {
  ...
  const readinessScore = Math.max(0, Math.min(100, Number(data.readinessScore) || 0));
  const educationScore = Math.max(0, Math.min(100, Number(data.educationScore) || 0));
  const profile = typeof data.profile === "object" && data.profile ? data.profile : {};
  if (!["cat", "dog"].includes(species) || !readinessScore) return Response.json({ error: "검사 결과를 확인해 주세요." }, { status: 400 });
  const [assessment] = await auth.db.insert(readinessAssessments).values({ memberId: auth.user.userId, species, profileJson: JSON.stringify(profile).slice(0, 12000), readinessScore, educationScore, passed: educationScore >= 80 }).returning();
