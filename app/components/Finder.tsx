...
export function Finder({ animals, modeOnly, initialTags = "" }: { animals: Animal[]; modeOnly?: Mode; initialTags?: string }) {
  ...
  return <div className={mode === "draw" ? "ff-drawing-workspace" : undefined}>
    {mode === "draw" && <><div className="ff-drawing-topbar ff-modern-drawing-topbar"><a href="/find" aria-label="그림 찾기 닫기">‹</a><div className="ff-drawing-toolbar" aria-label="그림 도구"><button type="button" className="ff-toolbar-tool" data-active={toolSheet === "brush"} aria-label="브러시 크기 선택" onClick={() => { setIsErasing(false); setToolSheet("brush"); }}><PrefixIcon svg={<IconPencilLine/>}/><span className="ff-tool-preview" style={{ width: 8 + brushSize * 3, height: 8 + brushSize * 3 }}/></button><button type="button" className="ff-toolbar-tool" data-active={toolSheet === "eraser" || isErasing} aria-label="지우개 크기 선택" onClick={() => { setIsErasing(true); setToolSheet("eraser"); }}><PrefixIcon svg={<IconEraserHorizlineLine/>}/><span className="ff-tool-preview ff-tool-preview-eraser" style={{ width: 8 + eraserSize * 3, height: 8 + eraserSize * 3 }}/></button><button type="button" className="ff-toolbar-color" style={{ background: brushColor.hex }} data-active={toolSheet === "color"} aria-label="색상 팔레트 열기" onClick={() => setToolSheet("color")}><PrefixIcon svg={<IconPaletteLine/>}/></button><button type="button" className="ff-toolbar-tool" data-active={toolSheet === "save"} aria-label="그림 저장 형식 선택" onClick={() => setToolSheet("save")}><PrefixIcon svg={<IconArrowDownHorizlineLine/>}/></button><button type="button" className="ff-toolbar-tool" aria-label="그림 공유" onClick={shareDrawing}><PrefixIcon svg={<IconAndroidshareLine/>}/></button></div></div><BottomSheetRoot open={toolSheet !== null} onOpenChange={(open) => { if (!open) setToolSheet(null); }}><BottomSheetContent title={toolSheet === "brush" ? "브러시 크기" : toolSheet === "eraser" ? "지우개 크기" : toolSheet === "color" ? "색상 선택" : "그림 저장"} showHandle className="ff-drawing-tool-sheet"><BottomSheetBody>{(toolSheet === "brush" || toolSheet === "eraser") && <div className="ff-drawing-size-options" role="group" aria-label={toolSheet === "brush" ? "브러시 크기" : "지우개 크기"}>{[1,2,3,4,5].map((size) => { const eraser = toolSheet === "eraser"; const active = eraser ? eraserSize === size : brushSize === size; return <button type="button" key={size} data-active={active} onClick={() => { if (eraser) { setEraserSize(size); setIsErasing(true); } else { setBrushSize(size); setIsErasing(false); } setToolSheet(null); }}><span className={eraser ? "ff-size-dot ff-size-dot-eraser" : "ff-size-dot"} style={{ width: 14 + size * 7, height: 14 + size * 7 }}/><strong>{size}pt</strong></button>; })}</div>}{toolSheet === "color" && <div className="ff-drawing-color-options" role="group" aria-label="색상 선택">{palette.map((color) => <button type="button" key={color.name} data-active={!isErasing && brushColor.name === color.name} style={{ background: color.hex }} aria-label={`${color.name} 색상`} onClick={() => { setBrushColor(color); setIsErasing(false); setToolSheet(null); }}/>)}</div>}{toolSheet === "save" && <div className="ff-drawing-save-options" role="menu"><button type="button" onClick={() => saveDrawing("png")}>PNG 이미지</button><button type="button" onClick={() => saveDrawing("jpeg")}>JPG 이미지</button><button type="button" onClick={() => saveDrawing("webp")}>WEBP 이미지</button></div>}</BottomSheetBody></BottomSheetContent></BottomSheetRoot></>}
