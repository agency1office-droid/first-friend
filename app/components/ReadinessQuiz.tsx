...
export function ReadinessQuiz({ onClose = () => {} }: { onClose?: () => void }) {
  ...
  return <div className={`ff-readiness ff-readiness-${phase}`}>
    ...
    {phase === "questions" && <section className="ff-readiness-chapter" aria-labelledby="readiness-question-title"><h2 id="readiness-question-title">{question.question}</h2><fieldset className="ff-quiz-question ff-quiz-question-single"><legend className="ff-visually-hidden">{question.question}</legend>{question.options.map((option, optionIndex) => <label key={option}><input type="radio" name={`readiness-chapter-${questionIndex}`} checked={answers[questionIndex] === optionIndex} onChange={() => setAnswers((current) => ({ ...current, [questionIndex]: optionIndex }))} /><span className="ff-quiz-option-label">{option}</span></label>)}</fieldset>{answers[questionIndex] !== undefined && <div className="ff-readiness-tip"><strong>💡 준비 팁</strong><p>{question.explanation}</p></div>}</section>}
