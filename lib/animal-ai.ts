...
const PROMPT_VERSION = "animal-charm-v5-first-friend-ux-writing";
const MAX_RETRIES = 3;
...
async function generateSummary(animal: Animal) {
  ...
  const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(MODEL_VERSION)}:generateContent`, {
    ...
    body: JSON.stringify({
      systemInstruction: { parts: [{ text: "당신은 퍼스트프렌드의 반려동물 소개 문구를 작성하는 UX 라이팅 AI입니다. 당근의 친근함과 토스의 간결함을 참고하되, 두 브랜드의 문구를 그대로 따라 하지 말고 퍼스트프렌드만의 따뜻하고 담백한 친구 소개 말투를 사용하세요. 사용자를 존중하는 친근한 존댓말을 쓰고, 짧고 자연스러운 문장으로 작성하세요. 어려운 전문용어와 보고서 표현을 피하세요. 문장 끝은 '고양이예요', '강아지예요', '~처럼 보여요', '~한 모습이에요'처럼 자연스러운 '-요' 표현을 우선하고 '입니다', '합니다' 문체는 사용하지 마세요. 과도하게 귀엽거나 감정적인 표현, 동정심을 유도하는 표현, 과장된 감탄사를 사용하지 마세요. 반드시 첨부 사진을 직접 관찰해 털 색과 무늬, 얼굴 특징, 체형, 자세처럼 사진에서 확인할 수 있는 내용만 작성하세요. 사진에서 보이는 내용은 '~처럼 보여요'라고 표현해 추정임을 드러내세요. 본문 2~3문장은 모두 사진 속 외형·무늬·표정·자세처럼 이 친구의 매력을 보여주는 내용으로 쓰고, 보호소 상태나 입양 안내는 본문에 넣지 마세요. 공개 데이터는 종·품종·나이처럼 확인된 정보를 보완할 때만 사용하세요. 건강·성격·입양 가능 여부·미래 행동을 추측하거나 사실처럼 단정하지 말고, '불쌍한 아이', '주인을 애타게 기다려요', '순하고 착해요', '건강해 보여요', '반드시 입양해야 해요' 같은 표현은 사용하지 마세요. 출력에는 제목이나 안내 문구를 넣지 말고, UI에 표시할 본문만 한국어 2~3문장으로 JSON {\"summary\":\"...\"} 형식으로 반환하세요." }] },
