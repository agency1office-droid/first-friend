...
export function ReadinessQuiz({ onClose = () => {} }: { onClose?: () => void }) {
  ...
  return <div className={`ff-readiness ff-readiness-${phase}`}>
    ...
    {phase === "species" && <section className="ff-readiness-species-page" aria-labelledby="readiness-species-title"><h1 id="readiness-species-title">어떤 친구를 만나고 싶나요?</h1><div className="ff-readiness-species-grid" role="group" aria-label="입양을 준비하는 동물"><button type="button" className="ff-readiness-species-choice" data-selected={species === "cat" || undefined} aria-pressed={species === "cat"} onClick={() => setSpecies("cat")}><span className="ff-readiness-species-emoji" aria-hidden>🐱</span><strong>고양이</strong><small>조용한 실내 생활을 준비해요</small></button><button type="button" className="ff-readiness-species-choice" data-selected={species === "dog" || undefined} aria-pressed={species === "dog"} onClick={() => setSpecies("dog")}><span className="ff-readiness-species-emoji" aria-hidden>🐶</span><strong>강아지</strong><small>산책과 활동 시간을 준비해요</small></button></div><div className="ff-readiness-tip"><strong>💡 꿀팁</strong><p>고양이와 강아지는 생활 환경과 돌봄 방식이 달라요. 지금 고른 친구에 맞춰 다음 내용을 안내해 드릴게요.</p></div></section>}
    ...
    <footer className={`ff-readiness-actions${phase === "intro" || phase === "species" ? " is-single" : ""}`}>{phase === "result" ? <><ActionButton variant="neutralWeak" onClick={() => { setPhase("intro"); setShowResult(false); setStep(0); }}><IconChevronLeftLine aria-hidden />다시 확인하기</ActionButton><ActionButton className="ff-grow" asChild><a href="/find">입양할 친구 찾기</a></ActionButton></> : phase === "intro" ? <ActionButton className="ff-grow" onClick={next}>시작하기</ActionButton> : phase === "species" ? <ActionButton className="ff-grow" onClick={next}>다음</ActionButton> : <><ActionButton variant="neutralWeak" onClick={previous}><IconChevronLeftLine aria-hidden />이전</ActionButton><ActionButton className="ff-grow" disabled={answers[questionIndex] === undefined} onClick={next}>{step === questions.length - 1 ? "결과 보기" : "다음"}</ActionButton></>}</footer>
  </div>;
}
