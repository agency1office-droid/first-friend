...
test("uses a contextual animal detail topbar", async () => {
  ...
  assert.match(bridge, /<AppBackButton fallback="\/"/);
  assert.doesNotMatch(bridge, /window\.location\.assign|readHomeFeedUrl/);
  ...
  const petKnowledgeConfig = await readFile(new URL("../lib/quiz/pet-knowledge.ts", import.meta.url), "utf8");
  const careReadinessConfig = await readFile(new URL("../lib/quiz/care-readiness.ts", import.meta.url), "utf8");
  const careReadinessFlow = await readFile(new URL("../app/components/CareReadinessFlow.tsx", import.meta.url), "utf8");
  const quizTypes = await readFile(new URL("../lib/quiz/types.ts", import.meta.url), "utf8");
  ...
  assert.match(quizRegistry, /badge: "상식 퀴즈"/);
  assert.match(quizRegistry, /title: "반려동물과\\n더 가까워지는 상식"/);
  assert.match(quizRegistry, /title: "반려동물과\\n더 가까워지는 상식",\s+lead: ""/);
  assert.match(quizRegistry, /petKnowledgeQuestions/);
  assert.match(quizRegistry, /careReadinessQuestions/);
  ...
  const readinessShare = await readFile(new URL("../app/quiz/adoption-prep/share/[result]/page.tsx", import.meta.url), "utf8");
  const readinessShareLib = await readFile(new URL("../lib/readiness-share.ts", import.meta.url), "utf8");
  ...
  assert.match(readinessQuiz, /className="ff-readiness-home"/);
  assert.match(readinessQuiz, /IconHouseLine/);
  assert.doesNotMatch(readinessQuiz, /ff-readiness-header-actions">\{previewEnabled/);
  assert.match(quizRegistry, /badge: "입양 준비 체크"/);
  assert.match(readinessQuiz, /반려동물과\\n함께할 준비하기/);
  ...
  assert.match(readinessQuiz, /data-quiz-id=\{quizDefinition\?\.slug \?\? "adoption-prep"\}/);
  assert.match(readinessQuiz, /quizDefinition\?\.slug === "care-readiness"/);
  assert.doesNotMatch(readinessQuiz, /ff-readiness-intro-visual/);
  assert.match(readinessQuiz, /ff-readiness-species-page/);
  assert.match(readinessQuiz, /<h2 id="readiness-species-title"><span className="ff-readiness-question-label"/);
  assert.doesNotMatch(readinessQuiz, /ff-readiness-tip|준비 팁/);
  assert.match(readinessQuiz, /if \(phase === "species"\)/);
  assert.match(readinessQuiz, /totalPages = questions.length \+ \(hasSpeciesPage \? 1 : 0\)/);
  assert.match(readinessQuiz, /보호소에 무엇부터 물어볼까요/);
  assert.match(readinessQuiz, /문이 잠기는 이동장이나 차량용 안전장비/);
  assert.match(readinessQuiz, /사료·간식·배변용품과 예방접종·중성화·예상 밖 진료비/);
  assert.match(readinessQuiz, /동물등록 정보는 어떻게 확인해야 할까요/);
  assert.match(readinessQuiz, /반려동물을 가족으로 맞이한다는 것은 무엇을 의미할까요/);
  assert.match(readinessQuiz, /과거 경험과 현재 마음을 모두 알 수는 없어요/);
  assert.match(readinessQuiz, /분양 비용이 없거나 적으면 가벼운 마음으로 시작해도 될까요/);
  ...
  assert.match(readinessQuiz, /closeToDetail\(\)/);
  const detailReturn = await readFile(new URL("../app/components/detailReturn.ts", import.meta.url), "utf8");
  assert.match(detailReturn, /return_to/);
  assert.match(detailReturn, /return_scroll/);
  assert.match(detailReturn, /window\.sessionStorage/);
  assert.match(detailReturn, /window\.opener/);
  ...
  assert.ok(readinessResultAsset.byteLength > 0);
  ...
  assert.match(readinessStyles, /\.ff-readiness\[data-quiz-id="pet-knowledge"\]\.ff-readiness-intro \{ background-image: url\("\/pet-knowledge-intro\.webp"\); \}/);
  ...
  assert.match(readinessStyles, /\.ff-readiness-actions \.seed-action-button \{[^}]*transition: none !important;[^}]*animation: none !important;/);
  assert.match(readinessStyles, /\.ff-readiness-header-actions \{[^}]*display: flex;[^}]*gap: 8px;/);
  assert.match(readinessStyles, /\.ff-readiness-question-count \{[^}]*fg-neutral-subtle[^}]*white-space: nowrap;/);
  assert.match(readinessStyles, /\.ff-adoption-test-trigger \{[^}]*color: var\(--seed-color-palette-static-white\);/);
  assert.match(readinessQuiz, /if \(phase === "species"\)/);
  assert.match(readinessQuiz, /key=\{`question-next-\$\{pendingAnswer === null \? "disabled" : "enabled"\}`\} size="large" variant="brandSolid" className="ff-grow" disabled=\{pendingAnswer === null\}/);
  ...
  assert.match(readinessStyles, /\.ff-readiness-chapter \{[^}]*gap: 24px;[^}]*margin-top: 24px;[^}]*padding: 0 16px;/);
  assert.match(readinessStyles, /\.ff-readiness-species-page \{[^}]*gap: 18px;[^}]*margin-top: 24px;[^}]*padding: 0 16px;/);
  assert.match(readinessStyles, /\.ff-readiness-chapter h2 \{[^}]*width: 100%;[^}]*max-width: none;[^}]*text-wrap: pretty;[^}]*word-break: keep-all;/);
  ...
  assert.match(readinessStyles, /\.ff-readiness-questions \.ff-quiz-question-single label:has\(input:focus-visible\)/);
  ...
  assert.match(readinessStyles, /\.ff-adoption-test-page > \.ff-readiness \{ width: 100%; height: 100%; min-height: 0; padding: 0 16px/);
  assert.match(readinessStyles, /\.ff-adoption-test-page > \.ff-readiness > section \{ flex: 1 1 auto; min-height: 0; overflow-y: auto;/);
  assert.match(readinessStyles, /\.ff-adoption-test-page > \.ff-readiness > \.ff-readiness-actions \{ position: relative; bottom: auto; flex: 0 0 auto;/);
  assert.match(readinessStyles, /\.ff-readiness-actions\.is-intro/);
  assert.match(readinessQuiz, /useState<Species \| null>\(null\)/);
  assert.match(readinessQuiz, /if \(phase === "intro"\) \{ setSpecies\(null\); setAnswers\(\{\}\); setFirstAnswers\(\{\}\); setFeedbackAnswer\(null\); setRetryAnswerHintsVisible\(false\); setStep\(0\); setPhase\(quizDefinition\?\.showSpeciesSelection === false \? "questions" : "species"\); return; \}/);
  assert.match(readinessQuiz, /const \[retryAnswerHintsVisible, setRetryAnswerHintsVisible\] = useState\(false\);/);
  assert.match(readinessQuiz, /if \(phase === "questions" && feedbackAnswer !== null\) \{ setFeedbackAnswer\(null\); setPendingAnswer\(null\); setRetryAnswerHintsVisible\(false\); setRevealedAnswer\(null\); return; \}/);
  assert.match(readinessQuiz, /IconPawprintFill/);
  assert.match(readinessQuiz, /const showCorrectPaw = retryAnswerHintsVisible && pendingAnswer === null && optionIndex === question.answer/);
  assert.match(readinessQuiz, /function restartQuiz\(\) \{[\s\S]*setRetryAnswerHintsVisible\(true\)/);
  assert.doesNotMatch(readinessQuiz, /힌트|questionHints|ff-readiness-question-hint/);
  assert.match(readinessQuiz, /revealedAnswer/);
  assert.match(readinessQuiz, /const nextAnswers = \{ \.\.\.answers, \[step\]: answer \};\s*setAnswers\(nextAnswers\);/);
  assert.match(readinessQuiz, /setAnswers\(nextAnswers\);\s*if \(step < questions\.length - 1\)/);
  assert.match(readinessQuiz, /const \[firstAnswers, setFirstAnswers\] = useState<Record<number, number>>\(\{\}\);/);
  assert.match(readinessQuiz, /const submittedAnswers = questions\.map\(\(_, index\) => answers\[index\] \?\? firstAnswers\[index\]\);/);
  assert.match(readinessQuiz, /const nextSubmittedAnswers = questions\.map\(\(_, index\) => nextAnswers\[index\] \?\? nextFirstAnswers\[index\]\);/);
  assert.match(readinessQuiz, /const finalAnswers = questions\.map\(\(_, index\) => answers\[index\] \?\? firstAnswers\[index\]\);/);
  assert.match(readinessQuiz, /async function skipFeedback\(\)/);
  assert.match(readinessQuiz, /const finalAnswers = questions\.map/);
  assert.match(readinessQuiz, /calculateEducation\(finalAnswers\)/);
  assert.match(readinessQuiz, /function restartQuiz\(\)/);
  ...
  assert.match(readinessShare, /generateMetadata/);
  assert.match(readinessShare, /openGraph/);
  ...
  assert.match(readinessQuiz, /src="\/cat-selection\.webp"/);
  assert.match(readinessQuiz, /src="\/dog-selection\.webp"/);
...
test("adds cached, deferred AI animal introductions without putting image bytes in storage", async () => {
  ...
  assert.match(page, /secondaryValue=\{detailAge\(animal\)\.actualAge\}/);
  assert.match(page, /value=\{detailAge\(animal\)\.value\}/);
...
test("renders public lost-animal and shelter surfaces", async () => {
  ...
  assert.match(lostHtml, /우리 동네 분실동물/);
...
test("keeps draw, photo, and condition journeys independent while draw uses perfect-freehand controls", async () => {
  ...
  assert.match(finder, /IconMagnifyingglassSparkleLine.*찾기/);
