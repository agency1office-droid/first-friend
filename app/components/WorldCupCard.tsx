...
export function WorldCupCard({ ref, headline, breed, number, meta, shelter, assets }: Props) {
  ...
  return <svg ref={ref} className="ff-worldcup-card" viewBox={`0 0 ${CARD_WIDTH} ${CARD_HEIGHT}`} role="img" aria-label={`퍼스트프렌드, ${headline}, ${originalName} 인연 카드`} fontFamily={FONT}>
    <defs>
      ...
      <clipPath id="wc-string-front"><path d="M-200-200H1000V210H930V600H-200Z" /></clipPath>
    ...
    <text x={540} y={286} textAnchor="middle" fontSize={fitFontSize(headline, 44, 824)} fontWeight={700} fill={COLOR.ink}>{headline}</text>
    {/* 주인공 사진 */}
    <rect x={128} y={325} width={824} height={640} rx={40} fill={COLOR.neutralWeak} />
