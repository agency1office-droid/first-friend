...
import { PUBLIC_ANIMAL_AGE_MAX, PUBLIC_ANIMAL_AGE_TICKS, PUBLIC_ANIMAL_WEIGHT_MAX, PUBLIC_ANIMAL_WEIGHT_TICKS } from "../../lib/animal-filter-ranges";
...
function SnapRange({ label, value, max, ticks, unit, onChange }: { label: string; value: [number, number]; max: number; ticks: readonly number[]; unit: string; onChange: (value: [number, number]) => void }) {
  const maxValue = value[1], point = maxValue === max ? 0 : maxValue + 1;
  return <div className="ff-filter-range">
    <div className="ff-filter-range-value"><strong>{point === 0 ? `모든 ${label}` : `${maxValue}${unit} 이하`}</strong></div>
    <input aria-label={`${label} 선택`} type="range" min="0" max={max + 1} step="1" value={point} onChange={event => { const next = Number(event.target.value); onChange(next === 0 ? [0, max] : [0, next - 1]); }} />
    <div className="ff-filter-range-ticks"> <span style={{ left: "0%" }} data-selected={point === 0 || undefined}>전체</span>{ticks.map(item => <span key={item} style={{ left: `${((item + 1) / (max + 1)) * 100}%` }} data-selected={maxValue === item || undefined}>{item}{unit}</span>)}</div>
  </div>;
}
...
export function AllAnimalFilters({ activeCount, filters, setFilter, resetFilters }: Props) {
  ...
  const [ageRange, setAgeRange] = useState<[number, number]>([0, PUBLIC_ANIMAL_AGE_MAX]), [weightRange, setWeightRange] = useState<[number, number]>([0, PUBLIC_ANIMAL_WEIGHT_MAX]), [breedQuery, setBreedQuery] = useState(""), [showBreeds, setShowBreeds] = useState(false);
  ...
  const openPanel = () => {
    ...
    const nextAge = filters.ageMin === 0 && filters.ageMax === PUBLIC_ANIMAL_AGE_MAX ? 0 : Math.min(PUBLIC_ANIMAL_AGE_MAX + 1, filters.ageMax + 1);
    const nextWeight = filters.weightMin === 0 && filters.weightMax === PUBLIC_ANIMAL_WEIGHT_MAX ? 0 : Math.min(PUBLIC_ANIMAL_WEIGHT_MAX + 1, filters.weightMax + 1);
    setAgeRange(nextAge === 0 ? [0, PUBLIC_ANIMAL_AGE_MAX] : [0, nextAge - 1]); setWeightRange(nextWeight === 0 ? [0, PUBLIC_ANIMAL_WEIGHT_MAX] : [0, nextWeight - 1]); setBreedQuery(""); setShowBreeds(Boolean(filters.species !== "all")); setError(""); setOpen(true);
  };
  useEffect(() => {
  ...
  const clearDraft = () => { setSpecies("all"); setBreeds([]); setSex([]); setNeutered([]); setAgeRange([0, PUBLIC_ANIMAL_AGE_MAX]); setWeightRange([0, PUBLIC_ANIMAL_WEIGHT_MAX]); resetFilters(); };
  const applied = [
    ...
    ...((ageRange[0] !== 0 || ageRange[1] !== PUBLIC_ANIMAL_AGE_MAX) ? [{ key: "age", label: `나이 ${ageRange[0]}~${ageRange[1]}살` }] : []), ...((weightRange[0] !== 0 || weightRange[1] !== PUBLIC_ANIMAL_WEIGHT_MAX) ? [{ key: "weight", label: `체중 ${weightRange[0]}~${weightRange[1]}kg` }] : []),
  ];
  const removeApplied = (key: string) => { if (key === "species") { setSpecies("all"); setBreeds([]); } else if (key.startsWith("breed:")) setBreeds(current => current.filter(item => `breed:${item}` !== key)); else if (key.startsWith("sex:")) setSex([]); else if (key.startsWith("neutered:")) setNeutered([]); else if (key === "age") setAgeRange([0, PUBLIC_ANIMAL_AGE_MAX]); else if (key === "weight") setWeightRange([0, PUBLIC_ANIMAL_WEIGHT_MAX]); };
    ...
    {open && <div className="ff-all-filter-overlay" role="dialog" aria-modal="true" aria-label="전체 필터">
      ...
      <div className="ff-all-filter-content ff-filter-reference-content">
        {loading && <div className="ff-all-filter-loading"><LoadingIndicator label="필터를 불러오는 중" /></div>}{error && <p className="ff-all-filter-error">{error}</p>}{options && <>
          ...
          <section className="ff-filter-reference-section"><SectionHeading title="나이" icon={<IconCalendarLine />} /><SnapRange label="나이" value={ageRange} max={PUBLIC_ANIMAL_AGE_MAX} ticks={PUBLIC_ANIMAL_AGE_TICKS} unit="살" onChange={setAgeRange} /></section>
          <section className="ff-filter-reference-section"><SectionHeading title="체중" icon={<IconCheckmarkScaleLine />} /><SnapRange label="체중" value={weightRange} max={PUBLIC_ANIMAL_WEIGHT_MAX} ticks={PUBLIC_ANIMAL_WEIGHT_TICKS} unit="kg" onChange={setWeightRange} /></section>
          <section className="ff-filter-reference-section"><SectionHeading title="성별" icon={<IconMalesymbolFemalesymbolLine />} /><div className="ff-filter-choice-grid">{orderedSex(options.sex).map(value => <button type="button" key={value} className="ff-filter-choice" data-selected={sex.includes(value) || undefined} onClick={() => { setSex(value === "미상" ? [] : [value]); }}><span className={`ff-filter-choice-icon ff-filter-sex-icon is-${value === "수컷" ? "male" : value === "암컷" ? "female" : "unknown"}`} aria-hidden>{sexIcon(value)}</span><span>{humanize(value)}</span></button>)}</div></section>
