...
export function AnimalGallery({ name, image, images = [] }: { name:string; image:string; images?:string[] }) {
  ...
  return <div className="ff-gallery">
    ...
    {available.length > 1 && <div className="ff-gallery-thumbs" aria-label={`${name}의 다른 사진`}>
      {available.map((src, index) => <button type="button" key={src} data-active={index === selected} onClick={() => setSelected(index)} aria-label={`${available.length}장 중 ${index + 1}번째 사진 보기`} aria-pressed={index === selected}><img src={src} alt="" loading="lazy"/><span>{index + 1}/{available.length}</span></button>)}
    </div>}
    <dialog ref={dialogRef} className="ff-image-dialog">
      <div className="ff-image-dialog-inner">
        ...
        {available.length > 1 && <div className="ff-image-dialog-nav">{available.map((src, index) => <button type="button" key={src} data-active={index === selected} onClick={() => setSelected(index)} aria-label={`${available.length}장 중 ${index + 1}번째 원본 사진 보기`}><img src={src} alt=""/><span>{index + 1}/{available.length}</span></button>)}</div>}
      </div>
    </dialog>
  </div>;
}
