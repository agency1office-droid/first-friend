-- 거리순 검색용 좌표를 숫자 두 개의 반복 계산 대신 공간 타입으로 보관합니다.
create schema if not exists extensions;
create extension if not exists postgis with schema extensions;

alter table public.public_animals
  add column if not exists shelter_geo extensions.geography(Point, 4326);

create or replace function public.refresh_public_animal_shelter_geo()
returns trigger
language plpgsql
set search_path = public, extensions
as $$
begin
  if new.shelter_lat between -90 and 90
     and new.shelter_lng between -180 and 180
     and not coalesce(new.approximate_shelter_location, true) then
    new.shelter_geo := ST_SetSRID(ST_MakePoint(new.shelter_lng, new.shelter_lat), 4326)::geography;
  else
    new.shelter_geo := null;
  end if;
  return new;
end;
$$;

drop trigger if exists trg_public_animals_shelter_geo on public.public_animals;
create trigger trg_public_animals_shelter_geo
before insert or update of shelter_lat, shelter_lng, approximate_shelter_location
on public.public_animals
for each row execute function public.refresh_public_animal_shelter_geo();

update public.public_animals
set shelter_geo = case
  when shelter_lat between -90 and 90
   and shelter_lng between -180 and 180
   and not coalesce(approximate_shelter_location, true)
  then ST_SetSRID(ST_MakePoint(shelter_lng, shelter_lat), 4326)::geography
  else null
end;

create index if not exists idx_public_animals_shelter_geo
  on public.public_animals using gist (shelter_geo)
  where active = true and shelter_geo is not null;
...
select c.id, c.name, c.species, c.breed, c.up_kind_cd, c.kind_cd, c.age, c.age_group, c.sex, c.region, c.shelter_id, c.shelter_name,
  c.shelter_address, c.shelter_phone, c.shelter_lat::double precision, c.shelter_lng::double precision, c.approximate_shelter_location,
  c.updated, c.updated_at, c.image_1, c.image_2, c.colors_json, c.traits_json, c.summary, c.health_json, c.life_json, c.match_reason,
...
order by case when p_sort = 'distance' then coalesce(c.calculated_distance, 1e15) end asc,
  case when p_sort <> 'distance' then c.updated_at end desc, c.id asc
