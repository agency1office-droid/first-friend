    ...
    {open && <div className="ff-all-filter-overlay" role="dialog" aria-modal="true" aria-label="전체 필터">
      ...
      <div className="ff-all-filter-content ff-filter-reference-content">
        {loading && <div className="ff-all-filter-loading"><LoadingIndicator label="필터를 불러오는 중" /></div>}{error && <p className="ff-all-filter-error">{error}</p>}{options && <>
          ...
          <section className="ff-filter-reference-section"><SectionHeading title={species === "dog" ? "강아지 크기" : species === "cat" ? "고양이 크기" : "크기"} /><div className="ff-filter-choice-grid">{sizeOptions(species).map(([value, label]) => <button type="button" key={value} className={`ff-filter-choice ff-filter-size-choice is-${value}`} data-selected={sizeGroup === value || undefined} onClick={() => setSizeGroup(sizeGroup === value ? "all" : value)}><strong>{label}</strong></button>)}</div></section>
          <section className="ff-filter-reference-section"><SectionHeading title="성별" /><div className="ff-filter-choice-grid">{orderedSex(options.sex).map(value => <button type="button" key={value} className="ff-filter-choice ff-filter-sex-choice" data-selected={sex.includes(value) || undefined} onClick={() => { if (value === "미상" || sex.includes(value)) setSex([]); else setSex([value]); }}><strong>{humanize(value)}</strong></button>)}</div></section>
          <section className="ff-filter-reference-section"><SectionHeading title="중성화 여부" /><div className="ff-filter-choice-grid">{[["yes", "중성화 완료"], ["no", "중성화 안 됨"]].map(([value, label]) => <button type="button" key={value} className="ff-filter-choice ff-filter-neuter-choice" data-selected={neutered.includes(label) || undefined} onClick={() => setNeutered(neutered.includes(label) ? [] : [label])}><strong>{label}</strong></button>)}</div></section>
        </>}
