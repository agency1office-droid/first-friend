...
export function ReadinessQuiz({ onClose = () => {} }: { onClose?: () => void }) {
  ...
  function previous() {
    ...
    if (phase === "questions" && answers[step] !== undefined) { setAnswers((current) => { const nextAnswers = { ...current }; delete nextAnswers[step]; return nextAnswers; }); setPendingAnswer(null); return; }
    if (phase === "questions" && step > 0) { setAnswers((current) => { const nextAnswers = { ...current }; delete nextAnswers[step - 1]; return nextAnswers; }); setStep((current) => current - 1); setPendingAnswer(null); return; }
  ...
  function renderFooter() {
    if (phase === "result") {
      return <ActionButton size="large" className="ff-grow" asChild><a href="/find">입양할 친구 찾기</a></ActionButton>;
    }
    if (phase === "intro") {
  ...
  return <div className={`ff-readiness ff-readiness-${phase}`}>
    ...
    {phase === "intro" && <section className="ff-readiness-intro-content" aria-labelledby="readiness-intro-title"><div className="ff-readiness-intro-badge">퍼스트프렌드 준비 가이드</div><h1 id="readiness-intro-title">반려동물과<br />함께할 준비하기</h1><p className="ff-readiness-intro-lead">입양 전 필요한 내용을 가볍게 확인해 보세요.</p><p className="ff-readiness-intro-note">총 4개 챕터 · 약 2분</p></section>}

    {phase === "species" && <section className="ff-readiness-species-page" aria-labelledby="readiness-species-title"><h2 id="readiness-species-title"><span className="ff-readiness-question-label" aria-hidden="true">Q.</span>어떤 친구를 만나고 싶나요?</h2><div className="ff-readiness-species-grid" role="group" aria-label="입양을 준비하는 동물"><button type="button" className="ff-readiness-species-choice" data-selected={species === "cat" || undefined} aria-pressed={species === "cat"} onClick={() => setSpecies("cat")}><span className="ff-readiness-species-emoji" aria-hidden>🐱</span><strong>고양이</strong></button><button type="button" className="ff-readiness-species-choice" data-selected={species === "dog" || undefined} aria-pressed={species === "dog"} onClick={() => setSpecies("dog")}><span className="ff-readiness-species-emoji" aria-hidden>🐶</span><strong>강아지</strong></button></div><p className="ff-readiness-species-description">선택한 친구에 맞춰 입양 전에 알아둘 내용을 확인해 볼게요.</p></section>}

    {phase === "questions" && <section className={`ff-readiness-chapter${hasAnswered ? " is-feedback" : ""}`} aria-labelledby="readiness-question-title">{hasAnswered ? <div className="ff-readiness-feedback-page"><div className="ff-readiness-feedback is-incorrect" role="status" aria-live="polite" aria-label="오답 확인"><span className="ff-readiness-feedback-mark" aria-hidden><IconXmarkLine /></span><strong className="ff-readiness-feedback-title">오답이에요!</strong><p className="ff-readiness-feedback-selected" aria-label="내가 고른 답변"><span>내가 고른 답변:</span><span>{question.options[selectedAnswer]}</span></p><p className="ff-readiness-feedback-detail">{question.explanation}</p></div></div> : <><h2 id="readiness-question-title"><span className="ff-readiness-question-label" aria-hidden="true">Q.</span>{question.question}</h2><fieldset className="ff-quiz-question ff-quiz-question-single"><legend className="ff-visually-hidden">{question.question}</legend>{question.options.map((option, optionIndex) => <label key={option}><input type="radio" name={`readiness-chapter-${questionIndex}`} checked={pendingAnswer === optionIndex} onChange={() => setPendingAnswer(optionIndex)} /><span className="ff-quiz-option-label">{option}</span></label>)}</fieldset></>}</section>}
    ...
    <footer className={`ff-readiness-actions${phase === "intro" || phase === "species" || phase === "questions" || phase === "result" ? " is-single" : ""}`}>{renderFooter()}</footer>
  </div>;
}
