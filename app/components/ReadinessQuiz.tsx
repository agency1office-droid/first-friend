...
export function ReadinessQuiz({ onClose = () => {} }: { onClose?: () => void }) {
  ...
  return <div className={`ff-readiness ff-readiness-${phase}`}>
    ...
    {phase === "species" && <section className="ff-readiness-species-page" aria-labelledby="readiness-species-title"><h1 id="readiness-species-title">어떤 친구를 만나고 싶나요?</h1><div className="ff-readiness-species-grid" role="group" aria-label="입양을 준비하는 동물"><button type="button" className="ff-readiness-species-choice" data-selected={species === "cat" || undefined} aria-pressed={species === "cat"} onClick={() => setSpecies("cat")}><span className="ff-readiness-species-emoji" aria-hidden>🐱</span><strong>고양이</strong></button><button type="button" className="ff-readiness-species-choice" data-selected={species === "dog" || undefined} aria-pressed={species === "dog"} onClick={() => setSpecies("dog")}><span className="ff-readiness-species-emoji" aria-hidden>🐶</span><strong>강아지</strong></button></div></section>}
    ...
    <footer className={`ff-readiness-actions${phase === "intro" || phase === "species" ? " is-single" : ""}`}>{phase === "result" ? <><ActionButton size="large" variant="neutralWeak" onClick={() => { setPhase("intro"); setShowResult(false); setStep(0); setSpecies(null); }}>다시 확인하기</ActionButton><ActionButton size="large" className="ff-grow" asChild><a href="/find">입양할 친구 찾기</a></ActionButton></> : phase === "intro" ? <ActionButton size="large" className="ff-grow" onClick={next}>시작하기</ActionButton> : phase === "species" ? <ActionButton size="large" className="ff-grow" disabled={species === null} onClick={next}>다음</ActionButton> : <><ActionButton size="large" variant="neutralWeak" onClick={previous}>이전</ActionButton><ActionButton size="large" className="ff-grow" disabled={answers[questionIndex] === undefined} onClick={next}>{step === questions.length - 1 ? "결과 보기" : "다음"}</ActionButton></>}</footer>
  </div>;
}
