...
export function SavedSearchManager() {
  ...
  useEffect(() => { let active = true; void fetch("/api/saved-searches").then(async response => ({ response, body: await response.json() })).then(({ response, body }) => { if (!active) return; if (response.status === 401) { location.href = "/signin-with-chatgpt?return_to=%2Fmypage%2Fsearches"; return; } if (response.ok) setItems(body.searches); else setError(body.error); }); return () => { active = false; }; }, []);
  async function toggle(item: Search, value: boolean) { const response = await fetch("/api/saved-searches", { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ id: item.id, alertsEnabled: value }) }); if (response.ok) setItems(current => current.map(row => row.id === item.id ? { ...row, alertsEnabled: value } : row)); }
  async function remove(id: number) { if (!confirm("이 알림 조건을 삭제할까요?")) return; const response = await fetch(`/api/saved-searches?id=${id}`, { method: "DELETE" }); if (response.ok) setItems(current => current.filter(row => row.id !== id)); }
