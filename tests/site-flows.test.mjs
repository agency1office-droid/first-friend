...
test('favorite mutation recovers after network failure and does not mistake one known favorite for the entire list',async t=>{
  ...
  t.after(()=>{globalThis.fetch=originals.fetch;globalThis.window=originals.window;delete globalThis.__siteHooks;delete globalThis.__siteFeedback;});
  ...
  assert.equal(button.props.disabled,false,'network failure unlocks retry');assert.equal(messages.length,1);
