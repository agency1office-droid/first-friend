...
import { IconCameraLine, IconArrowDownLine, IconArrowDownHorizlineLine, IconEraserHorizlineFill, IconEraserHorizlineLine, IconMagnifyingglassLine, IconMagnifyingglassSparkleLine, IconPictureLine, IconArrowCounterclockwiseCircularLine, IconSlider2HorizontalLine, IconPaletteFill, IconPaletteLine, IconPaintrollerFill, IconPencilLine, IconAndroidshareLine } from "@karrotmarket/react-monochrome-icon";
...
export function Finder({ animals, modeOnly, initialTags = "" }: { animals: Animal[]; modeOnly?: Mode; initialTags?: string }) {
  ...
  return <div className={mode === "draw" ? "ff-drawing-workspace" : undefined}>
    {mode === "draw" && <><div className="ff-drawing-topbar ff-modern-drawing-topbar"><a href="/find" aria-label="그림 찾기 닫기">‹</a><div className="ff-drawing-toolbar" aria-label="그림 도구"><button type="button" className="ff-toolbar-tool" data-active={toolSheet === "brush"} aria-label="브러시 크기 선택" onClick={() => { setIsErasing(false); setToolSheet("brush"); }}><PrefixIcon svg={<IconPaintrollerFill/>}/><span className="ff-tool-preview" style={{ width: 8 + brushSize * 2, height: 8 + brushSize * 2 }}/></button><button type="button" className="ff-toolbar-tool" data-active={toolSheet === "eraser" || isErasing} aria-label="지우개 크기 선택" onClick={() => { setIsErasing(true); setToolSheet("eraser"); }}><PrefixIcon svg={<IconEraserHorizlineFill/>}/><span className="ff-tool-preview ff-tool-preview-eraser" style={{ width: 8 + eraserSize * 2, height: 8 + eraserSize * 2 }}/></button><button type="button" className="ff-toolbar-tool ff-toolbar-color" data-active={toolSheet === "color"} aria-label="색상 팔레트 열기" onClick={() => setToolSheet("color")}><PrefixIcon svg={<IconPaletteFill/>}/><span className="ff-toolbar-swatch" style={{ background: brushColor.hex }} /></button><button type="button" className="ff-toolbar-tool" data-active={toolSheet === "save"} aria-label="그림 저장 형식 선택" onClick={() => setToolSheet("save")}><PrefixIcon svg={<IconArrowDownLine/>}/></button><button type="button" className="ff-toolbar-tool" aria-label="그림 공유" onClick={shareDrawing}><PrefixIcon svg={<IconAndroidshareLine/>}/></button></div></div><BottomSheetRoot open={toolSheet !== null} onOpenChange={(open) => { if (!open) setToolSheet(null); }}><BottomSheetContent title={toolSheet === "brush" ? "브러시 크기" : toolSheet === "eraser" ? "지우개 크기" : toolSheet === "color" ? "색상 선택" : "그림 저장"} showHandle className="ff-drawing-tool-sheet"><BottomSheetBody>{(toolSheet === "brush" || toolSheet === "eraser") && (() => { const eraser = toolSheet === "eraser"; const value = eraser ? eraserSize : brushSize; return <div className="ff-drawing-size-slider"><div className="ff-drawing-size-slider-heading"><span>{eraser ? "지우개 크기" : "브러시 크기"}</span><strong>{value}pt</strong></div><div className="ff-drawing-size-preview"><span className={eraser ? "ff-size-dot ff-size-dot-eraser" : "ff-size-dot"} style={{ width: 14 + value * 4, height: 14 + value * 4 }} /></div><input aria-label={`${eraser ? "지우개" : "브러시"} 크기`} type="range" min="1" max="10" step="1" value={value} onChange={(event) => { const next = Number(event.target.value); if (eraser) { setEraserSize(next); setIsErasing(true); } else { setBrushSize(next); setIsErasing(false); } }} /><div className="ff-drawing-size-slider-labels"><span>1pt</span><span>5pt</span><span>10pt</span></div></div>; })()}{toolSheet === "color" && <div className="ff-drawing-color-options" role="group" aria-label="색상 선택">{palette.map((color) => <button type="button" key={color.name} data-active={!isErasing && brushColor.name === color.name} style={{ background: color.hex }} aria-label={`${color.name} 색상`} onClick={() => { setBrushColor(color); setIsErasing(false); setToolSheet(null); }}/>)}</div>}{toolSheet === "save" && <div className="ff-drawing-save-options" role="menu"><button type="button" onClick={() => saveDrawing("png")}>PNG 이미지</button><button type="button" onClick={() => saveDrawing("jpeg")}>JPG 이미지</button><button type="button" onClick={() => saveDrawing("webp")}>WEBP 이미지</button></div>}</BottomSheetBody></BottomSheetContent></BottomSheetRoot></>}
