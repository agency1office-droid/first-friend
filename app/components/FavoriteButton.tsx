...
export function FavoriteButton({ animalId, animalName, initialSaved, onFavoriteChange, onRemoveRequest, className }: { animalId: string; animalName: string; initialSaved?: boolean; onFavoriteChange?: (saved: boolean) => void; onRemoveRequest?: (remove: () => Promise<void>) => void; className?: string }) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 401) {
          ...
          window.dispatchEvent(new CustomEvent("ff-login-request", { detail: { returnTo: location.pathname + location.search, description: "관심 친구는 로그인 후 등록할 수 있어요" } }));
      ...
      feedback.success(next ? "관심 친구에 등록했어요" : "관심 친구에서 뺐어요", next ? { actionLabel: "목록보기", onAction: () => { router.push("/mypage/favorites"); } } : undefined);
    } catch {
      readCache();
      if (scope !== cacheScope) return;
      const restored = confirmedChanges.get(animalId) ?? favoriteIds?.has(animalId) ?? previous;
  ...
  return <button type="button" className={className ? "ff-card-scrap " + className : "ff-card-scrap"} aria-pressed={saved} aria-busy={busy} aria-disabled={busy} aria-label={animalName + " " + (saved ? "관심 친구에서 빼기" : "관심 친구에 등록하기")} onClick={() => {
    if (lock.current || pendingChanges.has(animalId)) return;
