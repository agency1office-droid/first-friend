...
export function FavoriteAnimalGrid({ animals: initialAnimals }: { animals: Animal[] }) {
  ...
  return <>
    <div className="ff-favorite-filters" style={{ gridTemplateColumns: "minmax(0, 1fr) auto", alignItems: "center" }}>
      <div className="ff-animal-filter-wrap" style={{ margin: 0 }} aria-label="관심 친구 필터"><div className="ff-animal-filter-scroll" style={{ padding: "var(--seed-dimension-x0_5) 0" }}>
      ...
      <div className="ff-favorite-filter-summary" style={{ whiteSpace: "nowrap", textAlign: "right" }}><span role="status">{active ? `${animals.length}마리 중 ${filtered.length}마리` : `관심 친구 ${animals.length}마리`}</span></div>
    </div>
    ...
    </div> : <div className="ff-empty">{animals.length ? "선택한 조건에 맞는 친구가 없어요." : "아직 스크랩한 친구가 없어요."}</div>}
  </>;
}
