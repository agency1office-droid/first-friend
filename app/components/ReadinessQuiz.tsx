...
export function ReadinessQuiz({ onClose = () => {} }: { onClose?: () => void }) {
  ...
  const progressLabel = phase === "result" ? "확인 결과" : phase === "species" ? "반려동물 선택" : question?.chapter;
  return <div className={`ff-readiness ff-readiness-${phase}`}>
    {phase !== "intro" && <div className="ff-readiness-progress" role="progressbar" aria-label={`입양 준비 과정 ${pageNumber}/${totalPages}`} aria-valuemin={1} aria-valuemax={totalPages} aria-valuenow={pageNumber}><div style={{ width: `${pageNumber / totalPages * 100}%` }} /></div>}
    ...
    {phase !== "intro" && <div className="ff-readiness-progress-meta"><div className="ff-readiness-progress-heading"><strong>{progressLabel}</strong><span>{pageNumber}/{totalPages}</span></div><div className="ff-step-label">{phase === "result" ? "모든 챕터를 확인했어요" : phase === "species" ? "준비 시작하기" : "CHAPTER " + (step + 1)}</div></div>}

    {phase === "intro" && <section className="ff-readiness-intro" aria-labelledby="readiness-intro-title"><div className="ff-readiness-intro-icon" aria-hidden>🐾</div><div className="ff-readiness-chapter-kicker">입양 전에 꼭 확인해요</div><h1 id="readiness-intro-title">입양 전 준비 확인</h1><p className="ff-description">반려동물과 함께 살아가기 전에 꼭 알아야 할 내용을 가볍게 확인해 보세요.</p><div className="ff-readiness-intro-visual" aria-hidden><span>오늘부터 시작하는</span><strong>우리의 준비 이야기</strong><b>🐾</b></div><div className="ff-readiness-intro-details"><div><strong>총 4개 챕터</strong><span>생활 환경부터 평생 책임까지</span></div><div><strong>약 2분</strong><span>한 화면에 한 문항씩 진행해요</span></div></div></section>}

    {phase === "species" && <section className="ff-readiness-species-page" aria-labelledby="readiness-species-title"><div className="ff-readiness-chapter-kicker">입양할 친구를 먼저 골라주세요</div><h1 id="readiness-species-title">어떤 친구를 만나고 싶나요?</h1><p className="ff-description">선택한 친구에 맞춰 입양 전에 알아둘 내용을 준비해 드릴게요.</p><div className="ff-readiness-species-grid" role="group" aria-label="입양을 준비하는 동물"><button type="button" className="ff-readiness-species-choice" data-selected={species === "cat" || undefined} aria-pressed={species === "cat"} onClick={() => setSpecies("cat")}><span className="ff-readiness-species-emoji" aria-hidden>🐱</span><strong>고양이</strong><small>조용한 실내 생활을 준비해요</small></button><button type="button" className="ff-readiness-species-choice" data-selected={species === "dog" || undefined} aria-pressed={species === "dog"} onClick={() => setSpecies("dog")}><span className="ff-readiness-species-emoji" aria-hidden>🐶</span><strong>강아지</strong><small>산책과 활동 시간을 준비해요</small></button></div></section>}
    ...
    <footer className={`ff-readiness-actions${phase === "intro" ? " is-intro" : ""}`}>{phase === "result" ? <><ActionButton variant="neutralWeak" onClick={() => { setPhase("intro"); setShowResult(false); setStep(0); }}><IconChevronLeftLine aria-hidden />다시 확인하기</ActionButton><ActionButton className="ff-grow" asChild><a href="/find">입양할 친구 찾기</a></ActionButton></> : phase === "intro" ? <ActionButton className="ff-grow" onClick={next}>시작하기</ActionButton> : <><ActionButton variant="neutralWeak" onClick={previous}><IconChevronLeftLine aria-hidden />이전</ActionButton><ActionButton className="ff-grow" disabled={phase === "questions" && answers[questionIndex] === undefined} onClick={next}>{phase === "species" ? "다음" : step === questions.length - 1 ? "결과 보기" : "다음"}</ActionButton></>}</footer>
  </div>;
}
