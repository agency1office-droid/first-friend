...
export async function GET(request: Request) {
  ...
  const detailVariant = requestUrl.searchParams.get("variant") === "detail";
  ...
  try {
    ...
    const body = await sharp(Buffer.from(await response.arrayBuffer()), { failOn: "none" })
      .resize({ width: 480, height: detailVariant ? 450 : 480, fit: "cover", position: "center", withoutEnlargement: true })
