...
export async function GET() {
  const user=await getChatGPTUser();if(!user)return Response.json({error:"본인 확인이 필요합니다."},{status:401});const{data:rows}=await getSupabaseServerClient().from("readiness_assessments").select("*").eq("member_id",user.userId).order("completed_at",{ascending:false}).limit(1);const assessment=rows?.[0]||null;
...
export async function POST(request: Request) {
  ...
  if (!["cat", "dog"].includes(species) || !Array.isArray(data.answers) || ![4, 10, 14, 17].includes(data.answers.length)) return Response.json({ error: "검사 결과를 확인해 주세요." }, { status: 400 });
