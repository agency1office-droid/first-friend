-- 퍼스트프렌드 목록 검색 RPC
-- 2단계: 필터·거리 계산·정렬·키셋 페이지네이션을 PostgreSQL에서 수행합니다.
...
returns table (
  ...
  shelter_phone text,
  shelter_lat double precision,
  shelter_lng double precision,
  approximate_shelter_location boolean,
  updated text,
...
with calculated as (
  select
    a.*,
    case
      when p_lat between -90 and 90
       and p_lng between -180 and 180
       and a.shelter_lat between -90 and 90
       and a.shelter_lng between -180 and 180
  ...
  where (
    ...
    and (
      ...
      or c.calculated_distance > p_cursor_distance_meters
      or (c.calculated_distance = p_cursor_distance_meters and c.id > coalesce(p_cursor_id, ''))
    )
  ) or (
    p_sort <> 'distance'
    and (
...
select
  c.id, c.name, c.species, c.breed, c.up_kind_cd, c.kind_cd, c.age, c.age_group, c.sex,
  c.region, c.shelter_id, c.shelter_name, c.shelter_address, c.shelter_phone,
  c.shelter_lat::double precision, c.shelter_lng::double precision, c.approximate_shelter_location,
  c.updated, c.updated_at, c.image_1, c.image_2, c.colors_json, c.traits_json, c.summary,
  c.health_json, c.life_json, c.match_reason, c.process_state, c.calculated_distance,
  c.result_count
from counted c
order by
  case when p_sort = 'distance' then c.calculated_distance end asc nulls last,
  case when p_sort <> 'distance' then c.updated_at end desc nulls last,
  c.id asc
