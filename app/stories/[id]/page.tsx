...
export default async function StoryPage({params}:{params:Promise<{id:string}>}){
 ...
 <div className="ff-board-body">{story.body}</div><StoryActions key={story.postId} postId={story.postId} initialCounts={story.reactionCounts} initialReaction={mine} signedIn={Boolean(member)} returnTo={"/stories/"+story.id}/>{member?.role==="admin"&&<StoryAdminActions postId={story.postId}/>}<section className="ff-section"><Callout tone="neutral" description="정확한 동물 위치·급식 장소·개인 연락처를 발견하면 신고해 주세요."/></section></article>;
}
