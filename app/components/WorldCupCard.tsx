...
export function WorldCupCard({ ref, headline, breed, number, meta, shelter, assets }: Props) {
  // 세로 배분(1350): 흐린 사진 바탕 위 카드 40~1312(QR 아래 여백 40px). 하트 실이 화면 왼쪽 벽에서 나와 카드 안쪽 상단을 가로지르고 하트는 카드 상단 테두리 위로 살짝, 꼬리는 카드 오른쪽 테두리를 지나 끝나며, 그 아래 머리 → 큰 사진(648px) → 이름 → QR·보호소, 카드 아래 띠에 날짜·주소.
  ...
  return <svg ref={ref} className="ff-worldcup-card" viewBox={`0 0 ${CARD_WIDTH} ${CARD_HEIGHT}`} role="img" aria-label={`${headline}, ${breed} 인연 카드`} fontFamily={FONT}>
    <defs>
      <clipPath id="wc-photo"><rect x={216} y={344} width={648} height={648} rx={44} /></clipPath>
    ...
    {assets && <image href={assets.string} x={-60} y={-35} width={1110} height={393} transform="rotate(-4 495 162)" preserveAspectRatio="xMidYMid meet" />}
    {/* 머리: 워드마크 · 카드 번호(공고 번호 끝자리, 실 꼬리가 지나는 오른쪽 위를 피해 워드마크 옆) · 카드 종류 · 헤드라인 */}
    {assets && <image href={assets.wordmark} x={108} y={176} width={240} height={36} preserveAspectRatio="xMinYMid meet" />}
    {number && <text x={368} y={206} fontSize={26} fontWeight={600} fill={COLOR.subtle}>No. {number}</text>}
    <rect x={350} y={232} width={380} height={46} rx={23} fill={COLOR.brandWeak} />
    <text x={540} y={263} textAnchor="middle" fontSize={23} fontWeight={700} fill={COLOR.brand}>이상형 월드컵</text>
    <text x={540} y={322} textAnchor="middle" fontSize={fitFontSize(headline, 40, 880)} fontWeight={700} fill={COLOR.ink}>{headline}</text>
    {/* 주인공 사진 */}
    <rect x={216} y={344} width={648} height={648} rx={44} fill={COLOR.neutralWeak} />
    {assets && <image href={assets.photo} x={216} y={344} width={648} height={648} preserveAspectRatio="xMidYMid slice" clipPath="url(#wc-photo)" />}
    <rect x={216} y={344} width={648} height={648} rx={44} fill="none" stroke={COLOR.line} strokeWidth={2} />
