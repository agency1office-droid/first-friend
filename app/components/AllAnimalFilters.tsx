    ...
    {open && <div className="ff-all-filter-overlay" role="dialog" aria-modal="true" aria-label="전체 필터" onWheelCapture={event => event.stopPropagation()} onTouchMoveCapture={event => event.stopPropagation()}>
      ...
      <div className="ff-all-filter-content ff-filter-reference-content">
        {loading && <div className="ff-all-filter-loading"><LoadingIndicator label="필터를 불러오는 중" /></div>}{error && <p className="ff-all-filter-error">{error}</p>}{options && <>
          ...
          <section className="ff-filter-reference-section"><SectionHeading title="중성화 여부" icon={<IconCheckmarkScaleLine />} /><div className="ff-filter-choice-grid"><button type="button" className="ff-filter-choice ff-filter-neuter-choice" data-selected={!neutered.length || undefined} onClick={() => setNeutered([])}><strong>모두</strong></button>{[["yes", "중성화 완료"], ["no", "중성화 안 됨"]].map(([value, label]) => <button type="button" key={value} className="ff-filter-choice ff-filter-neuter-choice" data-selected={neutered.includes(label) || undefined} onClick={() => setNeutered(current => current.includes(label) ? current.filter(item => item !== label) : [...current, label])}><strong>{label}</strong></button>)}</div></section>
        </>}
