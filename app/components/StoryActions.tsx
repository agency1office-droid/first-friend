...
export function StoryActions({postId,initialCounts,initialReaction=null}:{postId:number;initialCounts:Counts;initialReaction?:Choice}){
 const [selected,setSelected]=useState<Choice>(initialReaction),[counts,setCounts]=useState(initialCounts),[open,setOpen]=useState(false),[reason,setReason]=useState(""),[reported,setReported]=useState(false),[pending,setPending]=useState(false),[error,setError]=useState("");
