...
export function CareReadinessFlow({ memberName = null, admin = false }: { memberName?: string | null; admin?: boolean } = {}) {
  ...
  return <div className={`ff-readiness ff-care-readiness${!started ? " ff-readiness-intro" : ""}`} data-quiz-id="care-readiness" data-care-step={!started ? "intro" : isResult ? "result" : currentStep}>
    ...
    {!started ? <section className="ff-readiness-intro-content" aria-labelledby="care-readiness-intro-title"><div className="ff-readiness-intro-badge">입양 환경 점검</div><h1 id="care-readiness-intro-title">반려동물과<br />함께할 수 있을까요?</h1>{admin && <ActionButton size="small" variant="neutralWeak" onClick={preview}>관리자 · 결과 화면 미리보기</ActionButton>}</section> : isResult ? <section className="ff-care-result" aria-labelledby="care-result-title">
      ...
      <CertificateResult ref={certRef} badge="입양 환경 점검 확인서" quiz="care-readiness" illustration={`/${species ?? "cat"}-selection.webp`} memberName={memberName} rows={[{ label: "준비도", value: `${percent}%` }, { label: "확인한 항목", value: `${readyCount}/${results.length}` }]} share={{ title: "퍼스트프렌드 입양 환경 점검", text: `${results.length}가지 항목 중 ${readyCount}가지를 확인했어요.` }} extraAction={<button type="button" onClick={retry}>다시 확인하기</button>} />
