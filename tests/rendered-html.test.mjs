...
test("uses a contextual animal detail topbar", async () => {
  ...
  assert.match(readinessStyles, /\.ff-readiness-actions:not\(\.is-intro\) \.seed-action-button:not\(:disabled\) \{[^}]*color: var\(--seed-color-fg-neutral\);[^}]*background: var\(--seed-color-bg-neutral-weak\);/);
  assert.match(readinessStyles, /\.ff-readiness-intro \.ff-readiness-actions \{[^}]*background: transparent;/);
  assert.match(readinessStyles, /\.ff-readiness-intro \.ff-readiness-actions \.seed-action-button:not\(:disabled\) \{[^}]*background: var\(--seed-color-bg-brand-solid\);/);
