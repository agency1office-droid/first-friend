...
export function FavoriteAnimalGrid({ animals: initialAnimals }: { animals: Animal[] }) {
  ...
  return <div className="ff-animal-grid">
    {animals.map(animal => <AnimalCard key={animal.id} animal={animal} layout="photo" initialSaved onFavoriteChange={saved => { if (!saved) setAnimals(current => current.filter(item => item.id !== animal.id)); }}/>) }
  </div>;
}
